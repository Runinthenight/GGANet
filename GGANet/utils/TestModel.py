...
def test_precess(MODEL, pbar, LOSS, DEVICE, FOLD_NUM, gcn_out):
    ...
    with torch.no_grad():
        for i, data in pbar:
            '''data preparation '''
            compounds, protein_seq, protein_graph, labels = data
            ...
            if isinstance(MODEL, list):
                ...
                for i in range(len(MODEL)):
                    predicted_scores = predicted_scores + \
                        MODEL[i](compounds, protein_seq, protein_graph)
                predicted_scores = predicted_scores / FOLD_NUM
            else:
                predicted_scores, sim_loss = MODEL(compounds, protein_seq, protein_graph, gcn_out)
            loss = LOSS(predicted_scores, labels) + sim_loss
...
def test_model(MODEL, dataset_loader, save_path, DATASET, LOSS, DEVICE,  dataset_class="Train", save=True, FOLD_NUM=1, gcn_out=True):
    test_pbar = tqdm(
        enumerate(
    ...
    T, P, loss_test, Accuracy_test, Precision_test, Recall_test, AUC_test, PRC_test = test_precess(
        MODEL, test_pbar, LOSS, DEVICE, FOLD_NUM, gcn_out)
    if save:
        if FOLD_NUM == 1:
            filepath = save_path + \
