...
class AtomEmbedding(pnn.Layer):
    ...
    def __init__(self, atom_names, embed_dim):
        ...
        self.embed_list = pnn.LayerList()
        for name in self.atom_names:
            embed = pnn.Embedding(
                    CompoundKit.get_atom_feature_size(name) + 5,
                    embed_dim, 
                    weight_attr=pnn.initializer.XavierUniform())
...
class AtomFloatEmbedding(pnn.Layer):
    """
    Atom Float Encoder
    """
...
class BondAngleFloatRBF(pnn.Layer):
    """
