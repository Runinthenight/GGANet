...
class GeoGNNBlock(pnn.Layer):
    ...
    def __init__(self, embed_dim, dropout_rate, last_act):
        ...
        self.norm = pnn.LayerNorm(embed_dim)
        self.graph_norm = GraphNorm()
        if last_act:
            self.act = pnn.ReLU()
        self.dropout = pnn.Dropout(p=dropout_rate)
...
class GeoGNNModel(pnn.Layer):
    ...
    def __init__(self, model_config={}):
        ...
        self.bond_embedding_list = pnn.LayerList()
        self.bond_float_rbf_list = pnn.LayerList()
        self.bond_angle_float_rbf_list = pnn.LayerList()
        self.atom_bond_block_list = pnn.LayerList()
        self.bond_angle_block_list = pnn.LayerList()
...
class GeoPredModel(pnn.Layer):
    ...
    def __init__(self, model_config, compound_encoder):
        ...
        if 'Cm' in self.pretrain_tasks:
            ...
            self.Cm_linear = pnn.Linear(compound_encoder.embed_dim, self.Cm_vocab + 3)
            self.Cm_loss = pnn.CrossEntropyLoss()
        # functinal group
        self.Fg_linear = pnn.Linear(compound_encoder.embed_dim, model_config['Fg_size']) # 494
        self.Fg_loss = pnn.BCEWithLogitsLoss()
        ...
        if 'Bar' in self.pretrain_tasks:
            ...
            self.Bar_loss = pnn.SmoothL1Loss()
        ...
        if 'Blr' in self.pretrain_tasks:
            self.Blr_mlp = MLP(2,
                    ...
                    out_size=1,
                    dropout_rate=self.dropout_rate)
            self.Blr_loss = pnn.SmoothL1Loss()
        ...
        if 'Adc' in self.pretrain_tasks:
            ...
            self.Adc_mlp = MLP(2,
                    ...
                    dropout_rate=self.dropout_rate)
            self.Adc_loss = pnn.CrossEntropyLoss()
