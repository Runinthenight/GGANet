...
def batch_norm_1d(num_channels):
    ...
    if dist.get_world_size() > 1:
        return pnn.SyncBatchNorm.convert_sync_batchnorm(pnn.BatchNorm1D(num_channels))
    else:
        return pnn.BatchNorm1D(num_channels)
...
def act_layer(act_type, inplace=False, neg_slope=0.2, n_prelu=1):
    ...
    if act == 'relu':
        ...
        layer = pnn.LeakyReLU(neg_slope, inplace)
    elif act == 'prelu':
        layer = pnn.PReLU(num_parameters=n_prelu, init=neg_slope)
    elif act == 'swish':
        layer = pnn.Swish()
    else:
...
def Linear(input_size, hidden_size, with_bias=True):
    ...
    fc_bias_attr = paddle.ParamAttr(initializer=pnn.initializer.Uniform(
    ...
    fc_w_attr = paddle.ParamAttr(initializer=pnn.initializer.Uniform(
    ...
    def __init__(self, config, with_efeat=True):
        ...
        if self.learn_t and self.aggr == "softmax":
            self.t = self.create_parameter(
                shape=[1, ],
                dtype='float32',
                default_initializer=pnn.initializer.Constant(value=self.init_t))
        else:
            self.t = self.init_t
        ...
        if self.learn_p:
            self.p = self.create_parameter(
                shape=[1, ],
                dtype='float32',
                default_initializer=pnn.initializer.Constant(value=self.init_p))
    ...
    def __init__(self, config, with_efeat=True):
        ...
        if self.virtual_node:
            ...
            self.virtualnode_embedding = self.create_parameter(
                ...
                dtype='float32',
                default_initializer=pnn.initializer.Constant(value=0.0))
        ...
        self.pool = gpnn.GraphPool(pool_type="sum")

        if not self.config["graphnorm"]:
            self.gn = gpnn.GraphNorm()
        
        hidden_size = self.emb_dim

        if self.config["clf_layers"] == 3:
            log.info("clf_layers is 3")
            self.graph_pred_linear = pnn.Sequential(
