...
class PretrainGNNModel(pnn.Layer):
    """
    ...
    def __init__(self, model_config={}):
        ...
        self.gnn_list = pnn.LayerList()
        self.norm_list = pnn.LayerList()
        self.graph_norm_list = pnn.LayerList()
        self.dropout_list = pnn.LayerList()
        for layer_id in range(self.layer_num):
            ...
            if self.norm_type == 'batch_norm':
                self.norm_list.append(pnn.BatchNorm1D(self.embed_dim))
            elif self.norm_type == 'layer_norm':
                self.norm_list.append(pnn.LayerNorm(self.embed_dim))
            else:
            ...
            self.dropout_list.append(pnn.Dropout(self.dropout_rate))
    ...
    def forward(self, graph):
        ...
        for layer_id in range(self.layer_num):
            ...
            if layer_id < self.layer_num - 1:
                node_feat = pnn.functional.relu(node_feat)
...
class AttrmaskModel(pnn.Layer):
    ...
    def __init__(self, model_config, compound_encoder):
        ...
        self.linear = pnn.Linear(compound_encoder.node_dim, out_size)
        self.criterion = pnn.CrossEntropyLoss()
...
class SupervisedModel(pnn.Layer):
    """
    ...
    def __init__(self, model_config, compound_encoder):
        ...
        self.linear = pnn.Linear(compound_encoder.graph_dim, self.task_num)
        self.criterion = pnn.BCEWithLogitsLoss(reduction='none')
